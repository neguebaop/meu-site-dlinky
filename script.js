...
function renderDash(){document.documentElement.style.setProperty('--neon',user.color||'#a855f7');$('#sideName').textContent=user.name;$('#sideUrl').textContent='dlinky/'+user.slug;$('#dashName').textContent=user.name;$('#dashSlug').textContent='@'+user.slug;$('#viewsCount').textContent=user.views||0;setBg($('#dashAvatar'),user.avatar);setBg($('#sideAvatar'),user.avatar);$('#cfgName').value=user.name;$('#cfgSlug').value=user.slug;$('#cfgBio').value=user.bio;$('#cfgMusic').value=user.music||'';$('#cfgWelcome').value=user.welcome||'Clique aqui';$('#cfgAvatar').value=user.avatar||'';$('#cfgBanner').value=user.banner||'';$('#cfgBg').value=user.bg||'';$('#cfgVideo').value=user.video||'';$('#cfgFrame').value=user.frame||'';$('#cfgColor').value=user.color||'#a855f7';$('#cfgParticleType').value=user.particleType||'snow';$('#cfgTemplate').value=user.template||'default';$('#cfgDecoration').value=user.decoration||'none';$('#cfgVerified').checked=!!user.verified;$('#cfgHideViews').checked=!!user.hideViews;$('#historyList').innerHTML=(user.history||[]).map(x=>`<li>${x}</li>`).join('')||'<li>Nenhum histórico ainda.</li>'}
